...
def build_parser():
    ...
    p.add_argument("--use_bn", dest="use_bn", action="store_true")
    p.add_argument("--no_bn", dest="use_bn", action="store_false"); p.set_defaults(use_bn=True)
    ...
    p.add_argument("--wandb", action="store_true"); p.add_argument("--wandb_project", type=str, default="vgg6-cifar10-assignment")
    p.add_argument("--wandb_group", type=str, default="q2q5-sweeps"); p.add_argument("--run_name", type=str, default=None)
    return p
...
def main():
    ...
    model = VGG6(num_classes=10, dropout=args.dropout, act_name=args.activation, use_bn=args.use_bn).to(device)
    ...
    with open(os.path.join(args.out_dir, "final_test_metrics.json"), "w") as f:
        json.dump({"test_loss": te_loss, "test_top1_acc": te_acc, "best_val_acc": best_val, "use_bn": args.use_bn}, f, indent=2)
    if wb:
        wb.summary["final_test_top1_acc"] = te_acc; wb.summary["best_val_acc"] = best_val; wb.summary["use_bn"] = args.use_bn; wb.finish()
    writer.close()
